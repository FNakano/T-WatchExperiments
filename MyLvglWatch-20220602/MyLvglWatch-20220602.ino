 ...
 *  Connects and start file upload but enters guru meditation and
 *  reboots. 20220605. FTP Example (without lvgl) also enters guru meditation
 *  
 */
...
//  start screen 
    ttgo->lvgl_begin();
    ttgo->bl->adjust(10); /* minimum brightness */
...
/* inspired in batmandial example */
    ...
    if ((tInactive>3000) && !FTPTestAndHandle()) { // boot time is aprox. 3s
        setCpuFrequencyMhz(10); // to save power
    } else {
        setCpuFrequencyMhz(240); // maximum power
    }
    if ((tInactive>10000) && !FTPTestAndHandle()) goDeepSleep();
    lv_task_handler();
    delay(5);
}
